...
def format_color(rgb):
    return "#%02x%02x%02x" % tuple(min(255, int(channel * 256)) for channel in rgb)
...
def relative_luma(rgb):
    return sum(
        coeff * c for (coeff, c) in zip([0.2126, 0.7152, 0.0722], parse_color(rgb))
    )
...
GTK_DIR = os.path.join(CWD, "common", "gtk-3.0")
...
BUILD_GTK_DIR = os.path.join(BUILD_DIR, "common", "gtk-3.0")
SVG_DIR = os.path.join(BUILD_GTK_DIR, "assets")
GTK_MAIN_FILE = os.path.join(BUILD_GTK_DIR, "gtk-%s.scss" % THEME_VARIANT)
